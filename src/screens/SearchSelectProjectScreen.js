...
import { getEmployeeList, getProjectsStats } from '../redux/authActions';
import { useDispatch, useSelector } from 'react-redux';
import { MaterialCommunityIcons } from '@expo/vector-icons';
const PAGE_SIZE = 10;

const SearchSelectProjectScreen = ({ onClose, selectedProjectId }) => {
    ...
    const [selectedProject, setSelectedEmployees] = useState(null);



    // useEffect(() => {

    //     // console.log(JSON.stringify(selectedProjectId))
    ...
    const fetchEmployee = useCallback((currentPage = 1, isRefresh = false) => {
        ...
        dispatch(getProjectsStats(userdata?.id,
            // employId?.employeeID,
            // currentPage,
            // PAGE_SIZE,
            // searchText, 
            (response) => {
                // console.log('Fetching page:', response.data, null, 2);
                if (response?.success) {
    ...
    useEffect(() => {
        if (selectedProjectId) {
            setSelectedEmployees(selectedProjectId);
        }
    }, [selectedProjectId]);
    ...
    const renderItem = ({ item }) => {
        ...
        const projectData = {
            ...
            clientName: category.client?.name || '',
            ...category
        };

        // ✅ Auto-selected if ID matches the one in consoleProjectData
        const isSelected = selectedProject?._id === projectData._id || projectData._id === selectedProjectId?._id;

        const handleSelect = () => {
            setSelectedEmployees(projectData);
            onClose(projectData);
        };
...
export default SearchSelectProjectScreen;
